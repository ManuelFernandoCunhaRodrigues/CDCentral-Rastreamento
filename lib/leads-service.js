...
const getSupabaseInsertKey = () => {
  ...
  if (serviceRoleKey && isProductionRuntime()) {
    throw new LeadStorageError("SUPABASE_LEADS_INSERT_KEY e obrigatoria em producao.", {
      code: "missing_supabase_leads_insert_key",
      statusCode: 500,
    });
  }

  if (serviceRoleKey && process.env.ALLOW_SUPABASE_SERVICE_ROLE_KEY_FALLBACK === "1") {
    return serviceRoleKey;
  }

  if (serviceRoleKey) {
    throw new LeadStorageError("Fallback para SUPABASE_SERVICE_ROLE_KEY exige opt-in explicito.", {
      code: "supabase_service_role_fallback_disabled",
      statusCode: 500,
    });
  }
